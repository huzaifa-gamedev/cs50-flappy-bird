...
-- Possible range for the interval between pipe spawns
local MIN_SPAWN_INTERVAL = 1.5
local MAX_SPAWN_INTERVAL = 2.9
...
function PlayState:update(dt)
    ...
    if love.keyboard.wasPressed('p') then
        ...
        if self.isPaused then
            love.audio.pause()  -- Pause all sounds
            sounds['pause']:play()  -- Play the pause sound effect
            scrolling = false  -- Stop background and ground scrolling
        else
            love.audio.play(sounds['music'])  -- Resume all sounds
            scrolling = true  -- Resume background and ground scrolling
        end
    end
...
function PlayState:render()
    ...
    if self.isPaused then
        -- Display a message indicating the game is paused and how to resume
